...
#Experiments where a training and test file are provided
#inConfigFile: csv input file that lists paraameters for each experiment, where each row defines an experiment to run. 
#outputDir: parent directory to store all output files of experiments
def runTrainTestDatasetExperiments(inConfigFile,outputDir):	
	
	#read the experiment configuration file 
	configDF = pd.read_csv(inConfigFile, sep=",")
	
	#make sure all needed columns are present
	myio.configFileIntegrityCheck(configDF,myio.TRAIN_TEST_DATASET_CONFIG_FILE_TYPE)
	...
	myio.logWrite("Starting ML experiments (specified training and test datsets) from experiment file "+inConfigFile,myio.LOG_LEVEL_INFO)
	...
	#create head
	resFile.write("experiment id,processing device,algorithm,"+myio.TRAIN_DATASET_YEAR_KEY+","+myio.TRAIN_DATASET_CHAMBER_KEY+","+myio.TEST_DATASET_YEAR_KEY+","+myio.TEST_DATASET_CHAMBER_KEY+","+myio.TEMPORAL_RESOLUTION_KEY+","+myio.INPUT_TENSOR_NUMBER_TIME_STEPS_KEY+","+myio.FEATURE_SELECTION_SCHEME_KEY+",number of features,number of training instances,number of testing instances,iteration,seed,inner fold,MSE,RMSE,R2,MAPE,execution time(s)\n")
	#for every experiment
	for eix in range(numExperiments):
		try:
			experimentStartTime=time.time()
			...
			myio.flushLogFile()
			#make sure the format of values for the experimetn are correct
			myio.configEntryIntegrityCheck(eix,configDF,myio.TRAIN_TEST_DATASET_CONFIG_FILE_TYPE)
			
			normalizeDataFlag = configDF[myio.APPLY_MIN_MAX_SCALING_FLAG_KEY][eix]
			
			
			inputTrainDatasetPath = configDF[myio.INPUT_TRAIN_DATASET_PATH_KEY][eix]
			inputTestDatasetPath = configDF[myio.INPUT_TEST_DATASET_PATH_KEY][eix]
			
			selectedFeaturePath=configDF[myio.SELECTED_SENSOR_PATH_KEY][eix]
			
			if not os.path.isfile(inputTrainDatasetPath):
				myio.logWrite("Could not find input train dataset "+str(inputTrainDatasetPath)+" for experiment "+str(eix)+". Skipping this experiment.",myio.LOG_LEVEL_ERROR)
				continue
			if not os.path.isfile(inputTestDatasetPath):
				myio.logWrite("Could not find input test dataset "+str(inputTestDatasetPath)+" for experiment "+str(eix)+". Skipping this experiment.",myio.LOG_LEVEL_ERROR)
				continue
			
			
			yearTrain =configDF[myio.TRAIN_DATASET_YEAR_KEY][eix]
			chamberTrain =configDF[myio.TRAIN_DATASET_CHAMBER_KEY][eix]
			yearTest =configDF[myio.TEST_DATASET_YEAR_KEY][eix]
			chamberTest =configDF[myio.TEST_DATASET_CHAMBER_KEY][eix]
			tempResolution =configDF[myio.TEMPORAL_RESOLUTION_KEY][eix]
			featSelectionScheme =configDF[myio.FEATURE_SELECTION_SCHEME_KEY][eix]
			tensorNumberTimeSteps = configDF[myio.INPUT_TENSOR_NUMBER_TIME_STEPS_KEY][eix]
			...
			#read the dataset into memory 
			inTrainDF,inTestDF,selectedFeatures = dataset.readTrainTestDataset(inputTrainDatasetPath,inputTestDatasetPath,selectedFeaturePath)
	
			myio.logWrite("First 3 samples of raw train dataset: "+str(inTrainDF.values[0:3,:]),myio.LOG_LEVEL_DEBUG)
			myio.logWrite("First 3 samples of raw test dataset: "+str(inTestDF.values[0:3,:]),myio.LOG_LEVEL_DEBUG)
			myio.logWrite("Last sample of raw train dataset: "+str(inTrainDF.values[-1,:]),myio.LOG_LEVEL_DEBUG)
			myio.logWrite("Last sample of raw test dataset: "+str(inTestDF.values[-1,:]),myio.LOG_LEVEL_DEBUG)
				
			
			originalTrainN2OData=inTrainDF.values[:,-1]
			originalTestN2OData=inTestDF.values[:,-1]
			
			#compute min and max n2o to  unscale the normalized
			#emissiosn for results presentation
			#since we normizle test data using train data min/max, use the min and max of train data
			maxN2O = max(originalTrainN2OData)
			minN2O = min(originalTrainN2OData)
			...
			if normalizeDataFlag:
				#normalize it
				normTrainDF = dataset.normalize(inTrainDF) #train normalized between 0 and 1
				normTestDF = dataset.normalize(inTestDF,inTrainDF) #normizle test data using train data min/max
			else:
				normTrainDF = inTrainDF
				normTestDF = inTestDF
							
			if model.isModelWithInputTensors(algName):
				#keep track of origional dataset before we remove reading that would lead to incomplete tensors (missing concecutive readigns)
				trimmedNormTrainDF = normTrainDF.copy(deep=True)  
				trimmedNormTestDF = normTestDF.copy(deep=True)  
				
				#remove rows were there is missing consecutive readings after to form a tensor
				#the sample ids don't change so that using sampleid as pointer to row in norm DF still possible
				dataset.removeNonConcurrentTensorSamples(trimmedNormTrainDF,tensorNumberTimeSteps,tempResolution)
				dataset.removeNonConcurrentTensorSamples(trimmedNormTestDF,tensorNumberTimeSteps,tempResolution)
								
			else:
				trimmedNormTrainDF=normTrainDF #they are the same for models that don't mind that some rows not concecutive
				trimmedNormTestDF=normTestDF #they are the same for models that don't mind that some rows not concecutive
					
			myio.logWrite("First 3 samples of normalized training dataset: "+str(trimmedNormTrainDF.values[0:3,:]),myio.LOG_LEVEL_DEBUG)
			myio.logWrite("First 3 samples of normalized test dataset: "+str(trimmedNormTestDF.values[0:3,:]),myio.LOG_LEVEL_DEBUG)
			myio.logWrite("Last sample of normalized training dataset: "+str(trimmedNormTrainDF.values[-1,:]),myio.LOG_LEVEL_DEBUG)
			myio.logWrite("Last sample of normalized test dataset: "+str(trimmedNormTestDF.values[-1,:]),myio.LOG_LEVEL_DEBUG)
			
			#number features
			nFeatures = len(selectedFeatures)
			if (len(trimmedNormTrainDF.columns)-3)!=nFeatures or (len(trimmedNormTestDF.columns)-3)!=nFeatures:
				raise Exception("Failed to properly read the dataset and select features.")
			...
			nTrainSamples = len(trimmedNormTrainDF.index) 
			nTestSamples = len(trimmedNormTestDF.index) 
			batchSize=None #all samples in the batch, since will easily fit into memory. I think this is unused
			numExecutionsPerTrial =  configDF[myio.NUMBER_OF_EXECUTIONS_PER_TRIAL_KEY][eix] 				
			
			myio.logWrite("Running experiment "+str(eix)+"/"+str(numExperiments-1)+" with algorithm "+algName+" using dataset with "+str(nTrainSamples)+" train samples, "+str(nTestSamples)+" test samples,  and "+str(nFeatures)+" features:\n ("+featuresStr+")." ,myio.LOG_LEVEL_INFO)
			...
			for itIx in range(configDF[myio.ITERATIONS_KEY][eix]):
				...
				trainIxs=mycommon.randomRowIndexShuffle(trimmedNormTrainDF)
				testIxs = []
				for i in range(len(trimmedNormTestDF.index)):
					testIxs.append(i)
				#testIxs=mycommon.randomRowIndexShuffle(trimmedNormTestDF)
				#we don't need to shuffle test samples to prepare for cross-val split
				
				
				inFoldIx=0
				trainDF = trimmedNormTrainDF.iloc[trainIxs]
				testDF = trimmedNormTestDF.iloc[testIxs]
					
				#todo: fix the outter loop indent
				#model has tensors?
				if model.isModelWithInputTensors(algName):
					#normDF given twice since we havne't take subset yet
					train_X,train_y,trainTimestamps=dataset.createTensorSets(normTrainDF,trimmedNormTrainDF,trainIxs,tensorNumberTimeSteps,tempResolution) 
					test_X,test_y,testTimestamps =dataset.createTensorSets(normTestDF,trimmedNormTestDF,testIxs,tensorNumberTimeSteps,tempResolution)
					...
					if len(train_X)==0:
						raise Exception("train (iteration-innerfold): "+str(itIx)+"-"+str(inFoldIx)+"- had an empty number of samples")						
					if len(test_X)==0:
						raise Exception("test (iteration-innerfold): "+str(itIx)+"-"+str(inFoldIx)+"- had an empty number of samples")						
							
							
					myio.logWrite("First sample of shuffled train data: "+str(train_X[0]),myio.LOG_LEVEL_DEBUG)
					myio.logWrite("First sample of shuffled test data: "+str(test_X[0]),myio.LOG_LEVEL_DEBUG)
					
					myio.logWrite("First sample of shuffled train Y: "+str(train_y[0]),myio.LOG_LEVEL_DEBUG)
					myio.logWrite("First sample of shuffled test Y: "+str(test_y[0]),myio.LOG_LEVEL_DEBUG)
					
					if train_X.shape[2] != nFeatures and train_X.shape[1] != tensorNumberTimeSteps:
						raise Exception("Failed to extract the training features as numpy matrix (some features missing")
					if test_X.shape[2] != nFeatures and test_X.shape[1] != tensorNumberTimeSteps:
						raise Exception("Failed to extract the testing features as numpy matrix (some features missing")
					
					
				else:
					
					train_X,train_y = dataset.extractTrainTestValues(trimmedNormTrainDF)					 
					test_X,test_y = dataset.extractTrainTestValues(trimmedNormTestDF)
					 
					myio.logWrite("First sample of shuffled train X: "+str(train_X[0,:]),myio.LOG_LEVEL_DEBUG)
					myio.logWrite("First sample of shuffled test X: "+str(test_X[0,:]),myio.LOG_LEVEL_DEBUG)
					
					
					if train_X.shape[1] != nFeatures:
						raise Exception("Failed to extract the  training features as numpy matrix (some features missing")
					if test_X.shape[1] != nFeatures:
						raise Exception("Failed to extract the  testing features as numpy matrix (some features missing")
					
				
				myio.logWrite("Shape of  train X: "+str(train_X.shape),myio.LOG_LEVEL_DEBUG)
				myio.logWrite("Shape of  test X: "+str(test_X.shape),myio.LOG_LEVEL_DEBUG)
				myio.logWrite("Shape of  train y: "+str(train_y.shape),myio.LOG_LEVEL_DEBUG)
				myio.logWrite("Shape of  test y: "+str(test_y.shape),myio.LOG_LEVEL_DEBUG)
				
				if normalizeDataFlag:
					deNormedOuterTest_y=dataset.denormalize(minN2O,maxN2O,test_y)
				else:
					deNormedOuterTest_y=test_y
				...
				mycommon.listAppend(iterationOutputDataMap[dataset.SAMPLE_ID_EXTRA_COLUMN_NAME],testDF[dataset.SAMPLE_ID_EXTRA_COLUMN_NAME])
				if model.isModelWithInputTensors(algName):
					mycommon.listAppend(iterationOutputDataMap[dataset.TIMESTAMP_COLUMN_NAME],testTimestamps) #timestamps offset by tensor length
				else:
					mycommon.listAppend(iterationOutputDataMap[dataset.TIMESTAMP_COLUMN_NAME],testDF[dataset.TIMESTAMP_COLUMN_NAME])
				mycommon.listAppend(iterationOutputDataMap["actual_N2O"],deNormedOuterTest_y)
				
				#get the shuffled indices of the samples that will be used by the cross validation splits to create folds				
				innerShuffledIndices = createRowIndexOrdering(configDF[myio.INNER_CV_SPLIT_TYPE_KEY][eix],trainDF,configDF[myio.TEMPORAL_INNER_CV_CLUSTER_SIZE_KEY][eix],configDF[myio.NUMBER_INNER_FOLDS_KEY][eix],0)#0 for timesteps since already delt with the last samples of incomplete timesteps
				...
				#for each inner fold innerTrain-valiation fold pairs
				for innerTrainIxs,innerValIxs in crossValSplit(configDF[myio.INNER_CV_SPLIT_TYPE_KEY][eix],trainDF,configDF[myio.NUMBER_INNER_FOLDS_KEY][eix],innerShuffledIndices):
					...
					#extract inner-train/validate datasets
					innerTrainDF = trainDF.iloc[innerTrainIxs]#CRASH OCCURING HERE FOR SOME REASON					
					innerValDF = trainDF.iloc[innerValIxs]
					
					#model has tensors?
					if model.isModelWithInputTensors(algName):
						innerTrain_X,innerTrain_y,innerTrainTimestamps=dataset.createTensorSets(normTrainDF,trainDF,innerTrainIxs,tensorNumberTimeSteps,tempResolution) 
						innerVal_X,innerVal_y,innerTestTimestamps=dataset.createTensorSets(normTrainDF,trainDF,innerValIxs,tensorNumberTimeSteps,tempResolution) 
						#make sure its [sample,timestep,feature]
						innerTrain_X = innerTrain_X.reshape((innerTrain_X.shape[0], tensorNumberTimeSteps, nFeatures))
						innerVal_X = innerVal_X.reshape((innerVal_X.shape[0], tensorNumberTimeSteps, nFeatures))
						if len(innerTrain_X)==0:
							raise Exception("A inner train fold (iteration-innerfold): "+str(itIx)+"-"+str(inFoldIx)+"- had an empty number of samples")							
						if len(innerVal_X)==0:
							raise Exception("A inner val fold (iteration-innerfold): "+str(itIx)+"-"+str(inFoldIx)+"- had an empty number of samples")							
						myio.logWrite("First sample of inner train X fold: "+str(innerTrain_X[0]),myio.LOG_LEVEL_DEBUG)
						myio.logWrite("First sample of inner val X fold: "+str(innerVal_X[0]),myio.LOG_LEVEL_DEBUG)
						myio.logWrite("First sample of inner train y fold: "+str(innerTrain_y[0]),myio.LOG_LEVEL_DEBUG)
						myio.logWrite("First sample of inner val y fold: "+str(innerVal_y[0]),myio.LOG_LEVEL_DEBUG)
					...
					if outputHyperParamsFlag:
						...
						outputHypeParams(bestHyperParamSet,firstHyperParamOutFlag,hyperParamFile,itIx,0,inFoldIx) #0 for outter fold is 0, since no outer CV with given train/test datsets
						#first time writing to file?
						if firstHyperParamOutFlag:
							firstHyperParamOutFlag=False
					...
					mlModel = model.Model(algName,bestHyperParamSet,inputShape)
					#train model
					mlModel.fit(train_X,train_y,test_X,test_y)
					
					preds = mlModel.predict(test_X)					
						
					r2 = mycommon.computeR2(test_y,preds)
					
					if normalizeDataFlag:
						mse = mycommon.computeMSE(dataset.denormalize(minN2O,maxN2O,test_y),dataset.denormalize(minN2O,maxN2O,preds))
						mape = mycommon.computeMAPE(dataset.denormalize(minN2O,maxN2O,test_y),dataset.denormalize(minN2O,maxN2O,preds),MAPE_SMALL_VALUE_OVERRIDE_THRESHOLD)
					else:
						mse = mycommon.computeMSE(test_y,preds)
						mape = mycommon.computeMAPE(test_y,preds,MAPE_SMALL_VALUE_OVERRIDE_THRESHOLD)
					...
					foldPairExecTime=foldPairEndTimeMS-foldPairStartTimeMS
					foldPairStartTimeMS=time.time()
					myio.logWrite("Experiment "+str(eix)+", algorithm "+algName+", (iteration)--(inner fold): ("+str(itIx)+")-("+str(inFoldIx)+")",myio.LOG_LEVEL_DEBUG)
					
					resFile.write(str(eix)+","+actualProcessingDevice+","+algName+","+str(yearTrain)+","+str(chamberTrain)+","+str(yearTest)+","+str(chamberTest)+","+str(tempResolution)+","+str(tensorNumberTimeSteps)+","+str(featSelectionScheme)+","+str(nFeatures)+","+str(nTrainSamples)+","+str(nTestSamples)+"," +str(itIx)+","+str(rngSeed)+","+str(inFoldIx)+","+str(mse)+","+str(rmse)+","+str(r2)+","+str(mape)+","+str(foldPairExecTime)+"\n")
					resFile.flush()
				...
				if TRAIN_TEST_MIX_INTEGRITY_CHECK:
					if len(trimmedNormTrainDF.index) != len(innerSampleTestCountMap):
						myio.logWrite("One of the samples in the inner CV was not part of a test set (expected count "+str(len(trimmedNormTrainDF.index))+" but was "+str(len(innerSampleTestCountMap))+"). Did you set an inner CV cluster size that was too small?",myio.LOG_LEVEL_ERROR)
					if len(trimmedNormTrainDF.index) != len(innerSampleTrainCountMap):
						myio.logWrite("One of the samples in the inner CV was not part of a train set (expected count "+str(len(trimmedNormTrainDF.index))+" but was "+str(len(innerSampleTrainCountMap))+") Did you set an inner CV cluster size that was too small?",myio.LOG_LEVEL_ERROR)
					
					expectedTrainParticipationCount = inFoldIx-1
					for k in innerSampleTrainCountMap:
						#make sure all samples were part of trainin equal 1 minus number of folds times
						if innerSampleTrainCountMap[k] !=  expectedTrainParticipationCount:
							myio.logWrite("One of the samples in the inner CV was not part of the training set every other train-test pair",myio.LOG_LEVEL_ERROR)
				...
				for k in iterationOutputDataMap:
					...
					if lastColLen != -1:
						...
						if lastColLen !=len(iterationOutputDataMap[k]):
							myio.logWrite("Issue outputing prediction file of an iteration due to differign length of column. Column "+str(k)+" has length "+str(len(iterationOutputDataMap[k]))+" but expected "+str(lastColLen),myio.LOG_LEVEL_ERROR)							
							differingColLenFlag=True
					else:
						lastColLen=len(iterationOutputDataMap[k])
				
				#PRINT the length of columns and names when differeing len
				if differingColLenFlag:
					for k in iterationOutputDataMap:						
						myio.logWrite("Column ('"+str(k)+"') with length: "+str(len(iterationOutputDataMap[k])),myio.LOG_LEVEL_DEBUG)							
...
if __name__ == '__main__':
	...
	ap.add_argument("-t", "--trainTestFileSpecific", type=str, required=True,
		help="flag when True indicates train and test files specific, and anything welse otherwise means cross-validation is ran on one input file.")
	args = vars(ap.parse_args())
		
	inputConfigFile = args["inFile"]
	outDir = args["outDirectory"]
	trainTestFlag = args["trainTestFileSpecific"]
	if trainTestFlag == "True":
		trainTestFlag=True
	else:
		trainTestFlag=False
	if trainTestFlag:
		runTrainTestDatasetExperiments(inputConfigFile,outDir)
	else:
